...
- 아이노드 객체와 파일 객체는 파일의 내용, 즉 파일 데이터에 접근하기 위해 사용된다

  - 아이노드 객체는 실제 파일의 내용을 담고 있는 디스크 블록에 대한 포인터를 포함하는 자료구조이고,
  - 파일 객체는 열린 파일에서 현재 접근되고 있는 데이터의 위치를 포함한다
  - 보통 파일 객체는 프로세스에만 속하지만, 아이노드 객체는 한 파일당 하나씩만 존재한다

- 디렉터리 파일들은 다른 파일들과 조금 다르게 다루어진다

  - 일반 파일들은 먼저 오픈해야 읽기나 쓰기가 가능하지만
  - 디렉터리 연산을 위한 시스템 콜들은 파일을 사전에 오픈하도록 요구하지 않는다
  - 그러므로 VFS는 이러한 디렉터리 연산을 파일 객체가 아니라 아이노드 객체에서 정의한다

- 슈퍼 블록 객체는 하나의 완전한 파일 시스템을 구성하기 위한 연결된 파일들의 집합을 나타낸다(전체 파일 시스템을 표현한다)

  - 운영체제 커널은 파일 시스템으로 마운트 된 디스크 장치마다 그리고 현재 연결된 네트워크 파일 시스템마다 각각 하나의 슈퍼블록 객체를 유지한다
...
- 마지막으로 디엔트리(dentry) 객체는 파일의 경로 중 디렉터리의 이름(/usr 같은) 또는 실제 파일(stdio.h 같은)을 저장하고 있는 `디렉터리 항목`을 나타낸다
  ...
  - 디엔트리 객체가 /usr/include/stdio.h 를 열려고 하는 상황
    ...
    - 경로 이름 해석은 시간을 매우 많이 소비하는 작업이므로, VFS는 이러한 해석을 최소화하기 위해 디엔트리 객체를 캐시에 저장한다
...
- Linux는 원래 Minix와 데이터 교환을 쉽게 하기 위해 Minix-호환 파일 시스템으로 만들어졌다
  ...
  - 이 절의 나머지 내용에서 현재까지 가장 널리 배포된 ext3를 검토한다. 대부분의 내용은 ext4에도 적용된다

- Linux의 ext3는 BSD Fast 파일 시스템(FFS)과 상당히 많은 공통점을 가진다
...
- Linux /proc 파일 시스템은 파일의 내용이 어디에도 실제로 저장되어 있지 않다
  - 대신 사용자가 /proc 파일의 입/출력을 요청하면 그때 가서 파일의 내용을 계산해 내는 특수한 파일 시스템의 예이다
...
- 블록 장치는 하드 디스크나 플로피디스크, 플래시 메모리와 같이 완전히 독립되고 고정된 크기의 데이터 블록에 임의 접근을 한다
...
- 블록 장치와 문자 자잋와는 조금 다르게 취급된다
- 사용자는 네트워크 장치에 직접적으로 데이터를 전송할 수 없고, 대신에 커널(kernel)의 네트워크 서브시스템(networking subsystem)과 연결을 설정한 후 이를 통해 간접적으로 통신해야 한다
...
- UNIX 파이프(pipe) 기법은 자식 프로세스가 통신 채널을 부모로부터 상속받게 한다
...
- Linux는 UNIX와 UNIX 간 통신에서 사용되는 인터넷 프로토콜을 지원하고, UNIX가 아닌 다른 운영체제를 위한 많은 프로토콜도 제공한다
- 특히, Linux는 큰 워크스테이션이나 서버급 시스템보다는 본래 PC용으로 구현되었기 때문에 Apple-Talk, IPX와 같은 PC 네트워크에서 사용되는 프로토콜을 대부분 지원한다

### Linux 커널 네트워크 세 층

- 내부적으로 Linux 커널에서 네트워킹은 세 층으로 이루어진 소프트웨어로 구현된다
...
- 사용자 응용 프로그램은 소켓 인터페이스를 통해 모든 네트워킹 요구 사항들을 수행한다
  ...
  - 버클리 BSD 4.3 소켓을 사용하여 어떤 프로그램도 Linux상에서 소스코드 변경없이 실행될 수 있도록 이 인터페이스는 설계되었다
  - `BSD 소켓 인터페이스`의 중요한 특징 중 하나는 매우 다양한 네트워킹 프로토콜에서 사용되는 네트워크 주소들도 다 나타낼 수 있도록 해준다는 것이다
...
- 다음 계층은 프로토콜 스택이다
  ...
  - 응용 프로그램 소켓으로부터(또는 네트워크 장치 드라이버로부터) 네트워킹 데이터가 이 계층에 도달할 경우 그 데이터는 사용 중인 네트워크 프로토콜이 어떤 것인가를 나타내는 식별자를 지니고 있도록 요구된다
  - 프로토콜들은 원할 경우 서로 통신을 할 수 있다
    - 예를 들어, 인터넷 프로토콜 안에는 어떻게 경로를 찾는지, 오류는 어떻게 처리할 것인지, 분실된 데이터를 어떻게 재전송할 것인지를 다루는 프로토콜들이 포함되어 있다
  - 각 프로토콜 계층에서 패킷을 재작성하거나, 생성할 수 있고, 패킷으로 나누거나 패킷들을 다시 합치기도 하고 들어오는 데이터를 단순히 버릴 수도 있다
...
- 네트워크 스택 계층 사이의 모든 통신은 하나의 skbuff(socket buffer) 구조체를 넘겨줌으로써 수행된다
...
- 네트워킹 스택의 프로토콜에 도착하는 패킷(skbuff)에는 그 패킷에 어떤 프로토콜이 사용되는지를 나타내는 내부 식별자(identifier)가 있다
  ...
  - 서로 다른 네트워킹 장치 드라이버는 프로토콜 유형을 인코딩 하는 방법이 다르다
  - 따라서 `네트워크로 부터 들어오는 데이터 프로토콜은 장치 드라이버에서 알아내야 한다`
  - 장치 드라이버는 이미 알려진 네트워킹 프로토콜 확인자들의 해시 테이블을 이용하여 적합한 프로토콜을 찾고 패킷을 그 프로토콜로 넘겨준다
  - 새로운 프로토콜을 추가하고 싶으면 커널 모듈 형태로 이 해시테이블에 추가하면 된다

- 들어오는 IP 패킷은 IP 드라이버로 전달된다
...
- IP 드라이버가 수행하는 다른 두 가지 기능은 큰 패킷을 해체하거나 재조립하는 일이다
...
- IP가 자기 자신 호스트가 목적지라고 확인한 패킷은 이 호스트 내의 적절한 프로토콜 드라이버로 보내진다
  ...
  - 연결된 소켓은 출발 주소와 도착 주소 그릭 ㅗ출발 포트번호와 도착 포트번호로 식별한다
  - 소켓 리스트는 이 네 주소로 인덱스된 해시테이블에 연결뙨다
  - 일단 패킷이 들어오면 출발/도착 주소와 포트번호 4개의 값을 키로 이 해시 테이블을 찾게 된다
  - 또, TCP 프로토콜은 통신 상태가 고르지 않을 경우도 대비해야 한다
...
- PAM(Pluggable Authentication Modules)
  - 인증 문제를 해결하기 위해 UNIX 사업자들이 새로운 보안 기법, PAM을 개발하였다
  - 사용자를 인증하기 위해 임의의 시스템 구성요소가 사용할 수 있는 공유 라이브러리 형태로 제공된다
