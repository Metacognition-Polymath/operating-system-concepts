...
- 호스트 id는 네트워크 번호와 호스트 번호로 나뉜다
  ...
  - IP 주소 클래스, 호스트 id, 네트워크 id
    ...
    - 최근에는 클래스 기반 IP 주소 할당 방식이 사용되지 않는다
      - 동적 호스트 구성 프로토콜을 사용한다(DHCP, Dynamic Host Configuration Protocol)
...
- 궁금한 점
  - HTTP3와 속도가 중요한 게임에선 UDP를 사용한다고 한다
    ...
    - 참고
      - https://http3-explained.haxx.se/ko/the-protocol/feature-reliable
      - https://velog.io/@bcj0114/HTTP-%EB%B2%84%EC%A0%84%EB%B3%84-%EB%B9%84%EA%B5%90-3-HTTP3
...
- DFS의 기본 아키텍처는 궁극적인 목표에 달려있다
  - 여기에서 논의할 두 아키텍처 모델
    - 클라이언트-서버 모델
      - 주요 목표 : 파일이 개별 클라이언트 시스템에 로컬로 저장된 것 처럼 하나 이상의 클라이언트 간에 투명한 파일 공유를 허용하는 것
      - 예시 : NFS(Network File System) version 3, OpenAFS
    - 클러스터 기반 모델
      - 고가용성과 확장성을 갖춘 대용량 데이터 세트를 대상으로 많은 응용 프로그램을 병렬로 실행해야 하는 경우
      - 예 : Google File System(GFS), Hadoop Distributed File System(HDFS)

### 19.6.1 클라이언트-서버 DFS 모델(The Client-Server DFS Model)

![19-12](./images/19-12.png)

- 그림 19.12 클라이언트-서버 DFS 모델

- 서버는 파일과 메타데이터를 모두 연결된 저장장치에 저장한다
- NFS(network file system) 프로토콜은 서버 장애 시 간단하고 빠른 크래시 복구를 초점으로 한다
- Andrew 파일 시스템(OpenAFS)은 확장성에 중점을 두고 만들어졌다
- OpenAFS와 NFS 모두 로컬 파일 시스템에 추가하여 사용된다
  - 다시 말해 NFS 파일 시스템 형식으로 하드 드라이브 파티션을 포맷하지 않는다
  - 대신 서버에서 클라이언트가 선택한 로컬 파일 시스템(ex. ext4)으로 파티션을 포맷하고 DFS를 통해 공유 디렉터리를 내보낸다
  - 클라이언트에서는 내보낸 디렉터리를 자신의 파일 시스템 트리에 붙이기만 하면 된다
  - 이러한 방식으로 DFS는 로컬 파일 시스템에 대한 책임에서 벗어날 수 있으며 분산 작업에 집중할 수 있다
- 설계에 따르면 DFS 클라이언트-서버 모델은 서버가 크래시 되면 단일 장애점(single point of failure) 문제를 겪을 수 있다
  - 컴퓨터 클러스터링은 중복 구성요소와 클러스터링 방법을 사용하여 이 문제를 해결할 수 있다
  - 클러스터링은 장애를 감지하고 작동중인 구성요소로 시스템 대체 작동하게 하여 서버 연산을 계속할 수 있게 한다
  - 또한 서버는 데이터와 메타데이터에 대한 모든 요청이 병목이 되어 확장성과 대역폭 문제를 초래한다

### 19.6.2 클러스터 기반 DFS 모델(The Cluster-Based DFS Model)

![19-13](./images/19-13.png)

- 그림 19.13 클러스터 기반 DFS 모델의 예
- 데이터, I/O 부하
...
- 호스트 이름을 네트워크 주소로 변환하려면 명명 시스템(e.g. DNS)을 사용해야 하며 네트워크 번호를 네트워크 장치 주소(e.g. 이더넷 주소)로 변환하려면 다른 프로토콜(ARP 등)이 필요할 수 있다
- 시스템이 별도의 네트워크에 있는 경우 소스 네트워크에서 대상 네트워크로 패킷을 전달하기 위해 라우터가 필요하다
- 전송 프로토콜 UDP 및 TCP는 고유한 시스템 전체 포트 번호를 사용하여 패킷을 대기 중인 프로세스로 보낸다
  - 또한 TCP 프로노콜은 패킷의 흐름이 신뢰할 수 있는 연결 지향 바이트 스트림이 되도록 한다
- 분산 시스템이 올바르게 작동하기 위해서는 극복해야 할 많은 과제가 있다
  - 문제는 시스템의 노드 및 프로세스 명명, 결함 허용, 오류 복구 및 확장성을 포함한다
  - 확장성 문제는 증가한 부하 처리, 결함 허용성 유 지, 압축 및 중복 제거 가능성을 포함한 효율적인 저장장치 기법 사용을 포함한다
- DFS는 클라이언트, 서버 및 저장장치가 분산 시스템의 사이트에 분산된 파일 서비스 시스템이다
  - 따라서 서비스 활동은 네트워크를 통해 수행되어야 한다
  - 하나의 중앙 집중식 데이터 저장소 대신 여러 개의 독립적인 저장장치가 있다
- DFS 모델에는 클라이언트 서버 모델과 클러스터 기반 모델의 두 가지 주요 유형이 있다
  - 클라이언트-서버 모델은 하나 이상의 클라이언트 간에 투명한 파일 공유를 허용한다
  - 클러스터 기반 모델은 파일을 하나 이상의 데이터 서버에 배포하고 대규모 병렬 데이터 처리를 위해 구축되었다
- 이상적으로 DFS는 클라이언트에게 전통적인 중앙 집중식 파일 시스템 처럼 보여야 한다(POSIX와 같은 전통적인 파일 시스템 인터페이스와 정확하게 일치하지는 않더라도).
  - 서버와 저장장치의 다양성과 분산은 투명해야 한다
  - 투명한 DFS는 클라이언트 환경을 클라이언트가 로그인한 사이트로 가져와서 클라이언트 이동성을 용이하게 한다
- DFS에는 명명 기법에 대한 몇 가지 접근 방식이 있다
  - 가장 간단한 방법으로 파일 이름은 호스트 이름과 로컬 이름의 조합으로 명명되며 시스템 전체에 걸쳐 고유한 이름을 보장한다
  - NFS에 의해 대중화된 또 다른 접근 방식은 원격 디렉터리를 로컬 디렉터리에 연결할 수 있는 수단을 제공하여 일관성 있는 디렉터리 트리 모양을 제공한다
- 원격 파일에 액세스 하기 위한 요청은 대개 두 가지 보완적 방법으로 처리된다
  - 원격 서비스를 사용하면 액세스 요청이 서버로 전달된다
  - 서버 기기는 액세스를 수행하고 그 결과는 클라이언트로 다시 전달된다
  - 캐싱을 사용하면 액세스 요청을 충족시키는 데 필요한 데이터가 아직 캐시되지 않은 경우 서버에서 클라이언트로 데이터 사본을 가져온다
  - 캐시된 사본에서 액세스가 수행된다
  - 캐시된 사본을 마스터 파일과 일관되게 유지하는 문제는 캐시 일관성 문제다
