...
- 호스트 - 입출력 하드웨어
  ...
  - 예를 들어, 컨트롤러와 호스트 사이에 생산자 소비자 관계를 조정하기 위해 두 개의 비트가 사용된다고 가정하자
    ...
    - 호스트는 다음과 같은 방법으로 핸드셰이킹을 통해 컨트롤러와 협력하면서 포트를 통해 출력을 쓴다
      ...
      - 3. 호스트가 명령 준비 완료 비트(command-ready bit)를 설정한다
    ...
    - 위 1단계에서 바쁜 대기(busy-waiting), 즉, 폴링(polling)을 하게 된다
      ...
      - 호스트가 폴링을 반복하고 있지만 장치가 서비스할 준비가 되는 데 시간이 오래걸린다면, 폴링은 비효율적인 연산이 되고,
...
- 현대 운영체제에서 세분화된 인터럽트 핸들링 방법이 필요하다
...
- 대부분 CPU는 두 종류의 인터럽트 요청 라인(interrupt request line)을 가진다
  ...
  - (2) 필요하면 인터럽트 기능을 잠시 중단 시켜놓을 수 있는 마스크 가능(maskable interrupt) 인터럽트이다
...
- 운영체제는 가상 메모리 페이징을 위해 인터럽트 기법을 사용한다
  - 페이지 폴트는 인터럽트를 발생시키는 가장 일반적인 이벤트이다
    - 페이지 폴트 발생 시 CPU는 하던 일을 미루고 페이지 폴트를 처리하는 핸들러로 달려간다
    - 이 핸들러는
      - 프로세스 현 상태를 저장하고,
...
- 정리
  ...
  - 인터럽트 구동 I/O는 이제 폴링보다 훨씬 일반적이며 폴링은 높은 처리량을 보이는 I/O에 사용된다
